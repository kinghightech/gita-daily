...
export default function GardenLevelScreen() {
  ...
  const [levelData, setLevelData] = useState<GardenLevelData | null>(null);
  ...
  const closeLesson = useCallback(() => {
    if (returnToPath === 'garden') {
      router.replace({ pathname: '/world-path/[slug]', params: { slug: 'garden' } });
      return;
    }
    router.back();
  }, [returnToPath]);
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const data = await fetchGardenLevel(levelNumber);
      if (cancelled) return;
  ...
  const finish = useCallback(async () => {
    ...
    if (passed) {
      await updateCurrentGardenLevel(levelNumber);
      const coinSource = isWisdomGate ? 'wisdom_gate' : 'world_level';
      void awardOmCoins(coinSource, `garden-${levelNumber}`).catch((err) => {
          ...
          >
            <Animated.View entering={FadeInUp.duration(500)} style={styles.wisdomContainer}>
              ...
              <View style={styles.wisdomIconBig}>
                <Flower2 size={56} color={GitaColors.gold} />
              </View>
              <Text style={styles.wisdomTitle}>Garden Complete</Text>
              <Text style={styles.wisdomSubheading}>Garden Path Wisdom Gate Passed</Text>
              <Text style={styles.wisdomMessage}>
                You studied action, duty, rebirth, and liberation as one living path. The
                seeds are planted; the practice continues.
              </Text>
              {questionTotal > 0 && (
      ...
      <View style={[styles.root, styles.center]}>
        ...
        <Animated.View entering={FadeInUp.duration(400)} style={styles.resultContainer}>
          <View style={[styles.resultIcon, passed ? styles.resultIconPass : styles.resultIconFail]}>
            <Flower2 size={44} color={passed ? GitaColors.gold : 'rgba(255,255,255,0.28)'} />
          </View>
...
const createStyles = (theme: Theme) =>
  StyleSheet.create({
    ...
    wisdomRoot: { backgroundColor: '#052E16' },
