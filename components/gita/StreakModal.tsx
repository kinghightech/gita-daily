...
export default function StreakModal({ open, onClose, preferences }: StreakModalProps) {
  ...
  const dragDownGesture = useMemo(
    () =>
      Gesture.Pan()
        .activeOffsetY(6)
        .failOffsetX([-22, 22])
        .onBegin(() => { canDrag.value = scrollOffsetY.value <= 0; })
        .onUpdate((event) => {
        ...
        .onEnd((event) => {
          ...
          sheetTranslateY.value = withSpring(0, { damping: 28, stiffness: 260, mass: 1 });
        }),
  ...
  useEffect(() => {
    ...
    sheetTranslateY.value = withTiming(screenHeight, { duration: 220, easing: Easing.in(Easing.cubic) }, (finished) => {
      if (finished) runOnJS(setIsMounted)(false);
    });
    closeFallbackTimerRef.current = setTimeout(() => { setIsMounted(false); closeFallbackTimerRef.current = null; }, 280);
  }, [isMounted, open, overlayOpacity, screenHeight, sheetTranslateY]);

  useEffect(() => {
    return () => { if (closeFallbackTimerRef.current) { clearTimeout(closeFallbackTimerRef.current); closeFallbackTimerRef.current = null; } };
  }, []);

  useEffect(() => {
    if (!isMounted) { overlayOpacity.value = 0; sheetTranslateY.value = screenHeight; }
                  ...
                  {calendarWeeks.map((week, weekIndex) => (
                    <View key={`week-${weekIndex}`} style={styles.weekRow}>
                      {week.map((day, dayIndex) => {
                        const dayStyle = day.isToday ? styles.dayToday
                          : day.isActive ? styles.dayActive
                          : day.isCurrentMonth ? styles.dayInactive
                          : styles.dayOutOfMonth;
                        const textStyle = day.isToday ? styles.dayTextToday
...
const createStyles = (theme: Theme) => StyleSheet.create({
  ...
  bottomWrap: { flex: 1, justifyContent: 'flex-end', alignItems: 'center' },
  sheet: {
    width: '100%',
    maxWidth: 448,
  ...
  sheetBg: { flex: 1, backgroundColor: theme.popup },
  dragHandle: { width: 40, height: 4, borderRadius: 999, backgroundColor: theme.border, alignSelf: 'center', marginTop: 10 },
  dragArea: { alignSelf: 'stretch', height: 74, justifyContent: 'flex-start' },
  headerIconBtn: { position: 'absolute', top: 14, padding: 6, zIndex: 10 },
  headerLeft: { left: 12 },
  ...
  dayInactive: { backgroundColor: theme.surface },
  dayOutOfMonth: { backgroundColor: theme.surface, opacity: 0.5 },
  dayActive: { backgroundColor: '#f59e0b' },
  dayToday: { backgroundColor: '#fbbf24', borderWidth: 3, borderColor: '#fcd34d' },
  dayTextBase: { fontSize: 12, fontWeight: '600' },
  dayTextInactive: { color: theme.subtext },
  dayTextOutOfMonth: { color: theme.subtextMuted },
  dayTextActive: { color: '#ffffff' },
  dayTextToday: { color: '#0f172a', fontWeight: '800' },
